...
/// Teacher email change — backend + realtime validation (theme-driven).
class TeacherEmailPage extends StatefulWidget {
  const TeacherEmailPage({super.key});
  @override
  State<TeacherEmailPage> createState() => _TeacherEmailPageState();
}

class _TeacherEmailPageState extends State<TeacherEmailPage> {
  // Sizes
  ...
  Future<void> _handleConfirm() async {
    ...
    try {
      ...
      // v5-safe: sends a verification email; Auth email updates after user confirms.
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              Text('Verification sent to $newEmail. Confirm from your inbox to finish.'),
          behavior: SnackBarBehavior.floating,
          backgroundColor: Theme.of(context).colorScheme.primary,
        ),
      );

      // Return a hint so the profile can show a pending state.
  ...
  /// Optional helper you can call later (e.g., when returning to profile)
  /// Mirrors the **already-changed** Auth email to Firestore.
  ...
  Widget build(BuildContext context) {
    ...
    // handy alpha util (avoids deprecated withOpacity warnings)
...
class _TightErrorRow extends StatelessWidget {
  final String text;
  final Color color;
  const _TightErrorRow({required this.text, required this.color});
  @override
  Widget build(BuildContext context) {
    return Padding(
...
class _TightOkRow extends StatelessWidget {
  final String text;
  final Color color;
  const _TightOkRow({required this.text, required this.color});
  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(top: 6),
