...
class TeacherNamePage extends StatefulWidget {
  const TeacherNamePage({super.key});
  @override
  State<TeacherNamePage> createState() => _TeacherNamePageState();
}

class _TeacherNamePageState extends State<TeacherNamePage> {
  ...
  Future<void> _prefillFromBackend() async {
    ...
    try {
      ...
      // Fallback to displayName split
      String? df, dl;
      final dn = user.displayName?.trim();
      if (dn != null && dn.isNotEmpty) {
        final parts = dn.split(' ');
        if (parts.isNotEmpty) df = parts.first;
        if (parts.length > 1) dl = parts.sublist(1).join(' ');
      }

      _firstC.text = (first?.isNotEmpty == true ? first! : (df ?? ''));
      _lastC.text = (last?.isNotEmpty == true ? last! : (dl ?? ''));

      setState(() {}); // refresh counters/borders
    } catch (_) {
  ...
  Future<void> _save() async {
    ...
    final first = _firstC.text.trim();
    final last = _lastC.text.trim();
    ...
    try {
      ...
      // Return values so TeacherProfile can refresh immediately
      Navigator.of(context).pop({'firstName': first, 'lastName': last});
    } on FirebaseException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(e.message ?? 'Failed to save name'),
          behavior: SnackBarBehavior.floating,
          backgroundColor: Theme.of(context).colorScheme.error,
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    // Responsive sizes
    final pillHeight = screenHeight * 0.06; // ~6% of screen height
    final pillHPad = screenWidth * 0.04; // ~4% of screen width
